...
class TsFoundLoadsScreen extends StatefulWidget {
  final String searchedLoadingPoint;
  final String searchedUnloadingPoint;
...
class _TsFoundLoadsScreenState extends State<TsFoundLoadsScreen> {
  ...
  Future<List<LoadDataModel>> getCardsData() async {
    ...
    List<LoadDataModel> sortedCard = [];
    for (int i = 0; i < card.length; i++) {
      if (card[i]
              .loadingPoint
              .toLowerCase()
              .contains(widget.searchedLoadingPoint.toLowerCase()) &&
          card[i]
              .unloadingPoint
              .toLowerCase()
              .contains(widget.searchedUnloadingPoint.toLowerCase())) {
        sortedCard.add(card[i]);
      } else if (card[i]
              .loadingPoint
              .toLowerCase()
              .contains(widget.searchedLoadingPoint.toLowerCase()) &&
          (widget.searchedUnloadingPoint.trim() == '' ||
              widget.searchedUnloadingPoint == null)) {
        sortedCard.add(card[i]);
      } else if (card[i]
              .unloadingPoint
              .toLowerCase()
              .contains(widget.searchedUnloadingPoint.toLowerCase()) &&
          (widget.searchedLoadingPoint == null ||
              widget.searchedLoadingPoint.trim() == '')) {
        sortedCard.add(card[i]);
      }
    }
    return sortedCard;
