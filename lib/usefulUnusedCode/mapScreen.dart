// code not in use as of now code just contains functions which might be useful in future
...
class _MapScreenState extends State<MapScreen> {
  ...
  void findDistanceBetweenPositions(
      Position initialPosition, Position finalPosition) async {
    http.Response response = await http.get(
        'https://apis.mapmyindia.com/advancedmaps/v1/5ug2mtejb2urr2zwgdg8l8mh3zdtm2i3/distance_matrix/driving/${initialPosition.longitude},${initialPosition.latitude};${finalPosition.longitude},${finalPosition.latitude}');
  ...
  void searchInputValueAndShowMarker(String value, String valueType) async {
    ...
    http.Response response = await http.get(
      'https://atlas.mapmyindia.com/api/places/geocode?address=$value&bias=1',
      headers: {HttpHeaders.authorizationHeader: "$token"},
    );
    print(response.statusCode);
    print(response.body);
    var eloc = (jsonDecode(response.body))["copResults"]["eLoc"];
    http.Response responseLat = await http.get(
        'https://apis.mapmyindia.com/advancedmaps/v1/5ug2mtejb2urr2zwgdg8l8mh3zdtm2i3/place_detail?place_id=$eloc');
  ...
  void getCurrentLocationAndAnimate() async {
    ...
    ); //camera moved to user's position
